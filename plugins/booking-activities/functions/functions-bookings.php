 ...
 * @return array
 */
function MBN_bookacti_get_booking_list_columns( $filters = array() ) {
	
	// Set up booking list columns
	$columns = apply_filters( 'bookacti_user_bookings_list_columns_titles', array(
		10	=> array( 'id' => 'activity',	'title' => esc_html__( 'Activity', BOOKACTI_PLUGIN_NAME )) ,
		20	=> array( 'id' => 'id',			'title' => esc_html__( 'id', 
 ...
 * @return array
 */
function MBN2_bookacti_get_booking_list_columns( $filters = array() ) {
 ...
 * @return string
 */
function MBN_bookacti_get_booking_list_rows( $filters, $columns = array() ) {
	
	if( ! $columns ) { $columns = MBN_bookacti_get_booking_list_columns( $filters ); }
	...
	// Build an array of bookings rows
	foreach( $bookings as $booking ) {
		//MBN START Get user infos
		
		//$user = null;
		if( ! empty( $users[ $booking->user_id ] ) ) {
	...
	foreach( $list_items as $list_item ) {
		...
		foreach( $columns as $column ) {
			...
			switch ( $column[ 'id' ] ) {
				case 'id':
					$value = isset( $list_item[ 'id' ] ) ? $list_item[ 'id' ]  : '';
					break;
				case 'activity':
					$value = isset( $list_item[ 'activity' ] ) ? $list_item[ 'activity' ] : '';
					break;
				case 'quantity':
					$value = isset( $list_item[ 'quantity' ] ) ?  $list_item[ 'quantity' ] : '';
					break;
				case 'state':
				case 'actions':
				default:
